...
function love.load()
    ...
    -- initialize mouse (touch) input table
...
--[[
    LÖVE2D callback fired each time a mouse button (touch) is pressed; gives us the
    X and Y of the mouse, as well as the button/touch in question.
]]
...
--[[
    Equivalent to our keyboard function from before, but for the mouse buttons/touch.
]]
...
function loadHighScores()
    ...
    love.filesystem.setIdentity('fifty_bird_ios')

    -- if file doesn't exist, initialize it with one score of 0
    if not love.filesystem.exists('fifty_bird_ios.lst') then
        love.filesystem.write('fifty_bird_ios.lst', '0\n')
    end

    -- iterate over each line of file. Should only be one line
    for line in love.filesystem.lines('fifty_bird_ios.lst') do
        score = tonumber(line)
    end
